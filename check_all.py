...
def print_quick_info(track):
    ...
    if not in_all:
        statuses.append("✘ all")
    if in_tempo != 1:
        statuses.append("✘ tempo" if in_tempo == 0 else "{in_tempo} tempo")
    if in_genre == 0:
        statuses.append("✘ genre")
    print(f"- {track.name} \033[90m{format_artists(track.artists)} \033[0;31m" + ", ".join(statuses) + "\033[0m")
...
sorter = PlaylistSorter(sp, prompt_for_all=True)
